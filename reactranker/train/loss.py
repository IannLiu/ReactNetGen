...
class GaussDisLoss(nn.Module):
    ...
    def forward(self, mean_scores, std_scores, targets, gpu: int):
        ...
        mse = 0.5 * torch.log(2 * self.pi) + 0.5 * torch.log(std_scores) + torch.pow((mean_scores - targets), 2) / (2 * std_scores)
...
class evidential_ranking(nn.Module):
    ...
    def forward(self,
        ...
        for item, batch_targets in zip(possibilities.split(scope, dim=0), targets.split(scope, dim=0)):
            """
            pred_possibility = item[:, 0]/torch.sum(item[:, 0])
            uncertainty_factor = item[:, 1]
            targets_score = torch.exp(batch_targets)
            targets_possibility = F.softmax(batch_targets, dim=0)
            Score_CrossEntropyLoss = -targets_possibility * torch.log(pred_possibility)
            err = torch.log(pred_possibility/targets_possibility)
            uncertainty_score = uncertainty_factor/(1-)
            Error_CrossEntropyLoss = -err * torch.log(uncertainty_score)
            annealing_coef = max_coeff * (epoch / (epochs - 1)) ** 3  # (epochs / (epoch + 1))
            loss = torch.mean(Score_CrossEntropyLoss + annealing_coef * Error_CrossEntropyLoss)
            """
            """
            evidential_score = item[:, 0]
            uncertainty_score = item[:, 1]
            targets_possibility = F.softmax(batch_targets, dim=0)
            total_score = torch.sum(item, dim=1)
            pred_possibility = total_score/torch.sum(total_score)
            Score_CrossEntropyLoss = -targets_possibility * torch.log(pred_possibility)
            # err = (1 - pred_possibility/targets_possibility) * total_score
            # Error_CrossEntropyLoss = -err * torch.log(uncertainty_score)
            err = torch.abs(1 - (targets_possibility / pred_possibility))
            Error_CrossEntropyLoss = -err * (torch.log(uncertainty_score)-torch.log(total_score))
            annealing_coef = max_coeff * (epoch / (epochs - 1)) ** 3  # (epochs / (epoch + 1))
            loss = torch.mean(Score_CrossEntropyLoss + annealing_coef * Error_CrossEntropyLoss)
            """
            evidential_score = item[:, 0]
            ...
            uncertainty_possibility = uncertainty_score / total_score
            err1 = (targets_possibility - pred_possibility - uncertainty_possibility) ** 2
