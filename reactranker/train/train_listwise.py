...
from .eval import evaluate_top_scores, calculate_ndcg, calculate_mse
...
def train(model: nn.Module,
    ...
    if task_type == 'mle_gaussian':
        ...
        listnet_loss = Listnet_For_evidential()
    ...
    for epoch in trange(epochs):
        ...
        for reactions_train, targets_train, scope_train in \
            ...
            if task_type == 'mledis_gaussian':
                ...
                betas = output[:, [j for j in range(len(output[0])) if j % 4 == 3]]
                variance = betas / (lambdas * (alphas - 1))
                loss = 0.2 * listnet_loss(mu, lambdas, alphas, scope_train, targets_train, gpu) +\
                       evidential_loss_new(mu, lambdas, alphas, betas, targets_train, gpu, lam=None)
        ...
        if save_metric == None or save_metric == 'average_score':
            ...
            if average_top1_in_pred >= score_old:
                ...
                save_checkpoint(path_checkpoints, model, mean, std)
                print('Note: the checkpint file is updated')
        elif save_metric == 'NDCG':
            ndcg, KL_div, order, smiles_and_index \
                = calculate_ndcg(model, gpu=gpu, data_processor=val_data_processor, smiles2graph_dic=smiles2graph_dic,
                                 batch_size=batch_size, NDCG_cut=0.5, smiles_list=smiles_list, target_name='std' + target_name,
                                 is_order=True, show_info=show_info)
            if ndcg >= score_old:
                score_old = ndcg
                save_checkpoint(path_checkpoints, model, mean, std)
                print('Note: the checkpint file is updated')
        ...
        if save_metric == 'NDCG':
            logger.info(
                'Epoch [{}/{}], NDCG,{:.4f}'.format(epoch + 1, epochs, score_old))
            print('Epoch [{}/{}], NDCG: {:.4f}'.format(epoch + 1, epochs, score_old))
        if save_metric == 'mse':
            logger.info(
