...
from rdradical.utils import PLEVEL, atoms_are_different
from rdradical.initialization import rdradicalReaction, rdradicalReactants
from rdradical.chiral import template_atom_could_have_been_tetra, copy_chirality,\
    atom_chirality_matches
from rdradical.clean import canonicalize_outcome_smiles, combine_enantiomers_into_racemic
from rdradical.bonds import BondDirOpposite, restore_bond_stereo_to_sp2_atom
...
def Hs_Num(mol):
    ...
    for atom in mol.GetAtoms():
        ...
        for nb in atom.GetNeighbors():
            if nb.GetSymbol() == 'H':
                nb_Hs += 1
        Hs = atom.GetNumExplicitHs() + atom.GetNumImplicitHs() + nb_Hs
        total_Hs += Hs     
    return total_Hs
"""
def Hs_Num(mol):
...
def rdradicalRunText(reaction_smarts, rad_rt_dic, rad_pt_dic, reactant_smiles, **kwargs):
    ...
    rxn = rdradicalReaction(reaction_smarts, rad_rt_dic, rad_pt_dic)
    reactants = rdradicalReactants(reactant_smiles)
    
    return rdradicalRun(rxn, reactants, **kwargs)


def rdradicalRun(rxn, reactants, keep_mapnums=True, combine_enantiomers=True, return_mapped=True):
    """
    ...
    if '.' in reactants.reactant_smiles:
        # print(reactants.reactants_achiral_list)
        outcomes = rxn.rxn.RunReactants(reactants.reactants_achiral_list)
        """
        print([Chem.MolToSmiles(reactant) for reactant in reactants.reactants_achiral_list])
        print(outcomes)
    ...
    if not outcomes and return_mapped:
        return [], {}, []
    elif not outcomes:
        return []
    ###############################################################################
    ...
    # Whether this reactant template is multi-molecules:
    multi_rt = rxn.multi_rt
    # If multi_rt, get the dic: atom map number -> which reactant belongs to
    if multi_rt:
        atomMapToReactantMap = rxn.atomMapToReactantMap
    ###############################################################################
    ...
    for outcome in outcomes:
        ...
        print('*'*80)
        for m in outcome:
            print('The ini outcomes: ', Chem.MolToSmiles(m))
        """
        ###############################################################################
        # Look for new atoms in products that were not in 
        # reactants (e.g., LGs for a retro reaction)
        # Reset the map number of outcomes (reacting atoms of outcomes loss the atom map number)
        if PLEVEL >= 2:
            print('Processing {}'.format(str([Chem.MolToSmiles(x, True) for x in outcome])))
        unmapped = 900
        """
        for m in outcome:
            print('test outcomes: ', Chem.MolToSmiles(m))
        """
        if multi_rt:
            for m in outcome:
                for a in m.GetAtoms():
                    if a.HasProp('old_mapno'):
                        ...
                        r_num = atomMapToReactantMap[int(a.GetProp('old_mapno'))]
                        react_idx = int(a.GetProp('react_atom_idx'))
                        react_num = reactants.reactants_achiral_list[r_num].GetAtomWithIdx(react_idx).GetAtomMapNum()
                        a.SetAtomMapNum(react_num)
            ...
            for m in outcome:
                for a in m.GetAtoms():
                    # Assign map number to outcome based on react_atom_idx
                    # print('outcomes: ', Chem.MolToSmiles(m))
        ...
        # Define map num -> reactant template atom map(reacting atom map number(new) -> template atom)
        ...
        # Make sure each atom matches by comparing the chirality of reacting atoms between reactants and reactant template.
        ...
        for match in (atom_chirality_matches(atoms_rt[i], atoms_r[i]) for i in atoms_rt):
            if match == 0: 
                if PLEVEL >= 2:
                    print('Chirality violated! Should not have gotten this match')
                skip_outcome = True 
                break
                ...
                if PLEVEL >= 2:
                    print('Part of the template matched reactant chirality, part is inverted! Should not match')
                skip_outcome = True 
                break
        if skip_outcome:
            continue 
        if PLEVEL >= 2:
            print('Chirality matches! Just checked with atom_chirality_matches')
        ...
        for atoms, dirs, is_implicit in reactants.atoms_across_double_bonds:
            if all(i in atoms_rt for i in atoms):
                ...
                if dirs != dirs_template and \
                    ...
                    if PLEVEL >= 5:
                        print('Reactant bond chirality does not match template!')
                    if PLEVEL >= 5:
                        print('Based on map numbers...')
                    if PLEVEL >= 5:
                        print('  rct: {} -> {}'.format(matched_atom_map_nums, dirs))
                    if PLEVEL >= 5:
                        print('  tmp: {} -> {}'.format(matched_atom_map_nums, dirs_template))
                    if PLEVEL >= 5:
                        print('skipping this outcome, should not have matched...')
                    skip_outcome = True 
                    break
        if skip_outcome:
            continue
        ...
        if len(mapnums) != len(set(mapnums)):  # duplicate?
            ...
            for j in range(1, len(outcome)):
                ...
                for b in new_mol.GetBonds():
                    ...
                    if PLEVEL >= 10:
                        print('stitching bond between {} and {} in stich has chirality {}, {}'.format(
            ...
            if PLEVEL >= 1:
                print('Merged editable mol, converted back to real mol, {}'.format(Chem.MolToSmiles(outcome, True)))
        else:
            ...
            outcome = new_outcome
        if PLEVEL >= 2:
            print('Converted all outcomes to single molecules')
        ###############################################################################
        ...
        for (i, j, b) in reactants.bonds_by_mapnum:
            # i is the atommapnum of begin atom, i is the atommapnum of end atom, b is bond
        ...
        if missing_bonds:
            if PLEVEL >= 1:
                print('Product is missing non-reacted bonds that were present in reactants!')
        ...
        try:
            # print('Before sanit: ', Chem.MolToSmiles(outcome))
            Chem.SanitizeMol(outcome)
            outcome.UpdatePropertyCache()
            # print('After sanit: ',Chem.MolToSmiles(outcome))
        ...
        # Check Hs
        # Some times, H number might be unbalanced
        total_ini_Hs = 0
        if '.' in reactants.reactant_smiles:
            rmols = reactants.reactants_achiral_list
            for rmol in rmols:
                total_ini_Hs += Hs_Num(rmol)
        else:
            rmol = reactants.reactants_achiral
            total_ini_Hs = Hs_Num(rmol)
        out_Hs = Hs_Num(outcome)
        if out_Hs != total_ini_Hs:
            # print('H number is unbalanced !!!, the unbalanced outcome is {}'.format(Chem.MolToSmiles(outcome)))
            continue
        # print(Chem.MolToSmiles(outcome))
        ###############################################################################
        ...
        for a in outcome.GetAtoms():
            ...
            if not a.HasProp('old_mapno'):
                ...
                if not a.HasProp('react_atom_idx'):
                    ...
                    #print(Chem.MolToSmiles(atoms_r[a.GetAtomMapNum()].GetOwningMol()))
        ...
        for b in outcome.GetBonds():
            ...
            H_number_ba = nb_H_Num(ba)
            H_number_bb = nb_H_Num(ba)
            if ba.GetDegree() == 1 or H_number_ba ==2 or bb.GetDegree() == 1 or H_number_bb == 2:
                continue
            ...
            # Start with setting the BeginAtom. Returns True if a bond direction was copied
        ...
        # Maybe, the outcome should be preserved without chirality.
        skip_outcome = False
        ...
        mapped_outcomes_dup.append(mapped_outcome)
    ###############################################################################
    ...
    if return_mapped:
        ...
        return list(final_outcomes), mapped_outcomes, mapped_outcomes_dup
    else:
...
if __name__ == '__main__':
    ...
    rad_rt_dic = None
    rad_pt_dic = None
    outcomes = rdradicalRunText(reaction_smarts, rad_rt_dic, rad_pt_dic, reactant_smiles)
    print(outcomes)

    # Pre-initialize
    rxn = rdradicalReaction(reaction_smarts, rad_rt_dic, rad_pt_dic,)
    reactants = rdradicalReactants(reactant_smiles)
    outcomes = rdradicalRun(rxn, reactants)
    print(outcomes)

    # Get list of atoms that changed as well
    outcomes, mapped_outcomes = rdradicalRun(rxn, reactants, return_mapped=True)
